...
class Topo2Ltopo:
    ...
    @classmethod
    def write_ltopo(cls, topo: Topography, det, out_fname='', calc_all=False):
        ...
        h_ltopo_thetaphi = topo.calc_path_length(det, _h_accept_thetaphi, 'thetaphi', calc_all)
        h_ltopo_thetaphi.SetNameTitle(dic_hist_name['hthetaphi_ltopo'],
                                      '%s; #phi [rad]; #theta [rad]' % dic_hist_name['hthetaphi_ltopo'])
        h_ltopo_thetaxy = topo.calc_path_length(det, _h_accept_thetaxy, 'thetaxy', calc_all)
        ...
        logger.info("Write Ltopo into %s" % out_fname)
        ...
        print('%s, %s done.' % (topo.get_topo_name(), det.get_det_name()))

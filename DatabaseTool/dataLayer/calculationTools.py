...
def ToEnergySpectrumData(data: pd.DataFrame) -> np.array:
    ...
    :param data:原始数据
        ------------INPUT STRUCTURE------------
                    0~35            36         37      38       39
        COLUMNS: chn_00~chn_35 temperature triggerID boardID  timeTag
        shape:  (None,40)
    :return:
        ------------OUTPUT STRUCTURE-----------
        shape:(8, 36, 4095)
...
def ChooseGoodEvent(data: pd.DataFrame) -> pd.DataFrame:
    ...
    :param data:
    :return: shape = (None, 39),triggerID变为Index
    '''
    triggerID = np.unique(data[_Index[37]].values)
    boardNum = np.unique(data[_Index[38]].values).shape[0]
...
# 计算全部板都触发的事件个数
def allTrigger(rawData: pd.DataFrame):
    boardNumber = rawData["boardID"].max()+1
    boardCount = rawData[["triggerID","boardID"]].groupby("triggerID").count().values
    AllTriggerNumber = boardCount == boardNumber
    return AllTriggerNumber.sum()

# 计算各板平均温度
def getAvergTemperature(rawData: pd.DataFrame):
    rawData["temperature_Celsius"] = translateTemperature(rawData["temperature"].values)
    l = rawData[["temperature_Celsius", "boardID"]].groupby("boardID").mean()
    return l.values.reshape((-1,))

# 计算温度和计数信息
def calculateForTempAndCountRate(rawData: pd.DataFrame):
    result = {}
    result["boardNumber"] = rawData["boardID"].max() + 1
    result["timeTagEnd"] = rawData["timeTag"].values[-1]
    result["timeTagStart"] = rawData["timeTag"][0]
    result["temperature"] = getAvergTemperature(rawData)
    result["count"] = allTrigger(rawData)
    return result

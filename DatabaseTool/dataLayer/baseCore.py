    ...
    def getData(self, index: int = -1, startIndex: int = 0, dtype = 'int64') -> pd.DataFrame:
        ...
        if index == 0:      #选择第一个数据集时
            if self.index.shape[0] == 1:
                ...
                stop = self.index[1]
            if startIndex >= stop:
                _d = pd.DataFrame(columns=_Index).astype(dtype)
            else:
                ...
                timetag = np.zeros((stop - startIndex,), dtype='int64')
                try:
            ...
            if startIndex >= stop:
                ...
                timetag = np.zeros((stop - start,), dtype='int64')
                timetag[-1] = 1
                try:

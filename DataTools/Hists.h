...
class Hists
{
	public:
		...
		**********************************************************************/
		...
		/**********************************************************************
		* Integrate histogram in range [x0,x1] (ROOT can do only bin#0,bin#1)
		* It does not take into account the position of x inside the bin,
		* it just uses TH1::FindBin(x).
		* If binWidth==true, the sum of bin contents is multiplied by bin width.
		**********************************************************************/
		static float Integral(TH1* h,float x0,float x1,bool binWidth = false);
		/**********************************************************************
...
ComPlot* compare_hist_list(TCanvas *c, int nhist, TList *lis_h)
{
    TVirtualPad *pad=0;
    ComPlot *comparison = new ComPlot(pad,nhist,"comp_name","","");
    comparison->Add((TH1F*)lis_h->At(0),"",Plot::Fill);
    comparison->Add((TH1F*)lis_h->At(1),"",Plot::Fill);

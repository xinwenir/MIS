...
class Make_refcence_tools():
    ...
    def save_refs(self, path, air_value, wall_value, inner_value):  #####写对应的设置文件
        ...
        :return: 定值True
        """
        from DataTools.Jxyz_Tools import getj_from_xyz
        shape = self.shape
        airj = self.airj
...
if __name__ == '__main__':
    ...
    datamanage = DataFactory.DataManage.DataManager()
    # setting=Setting.Setting()
    tool = Make_refcence_tools(datamanage.get_unneed_j(), datamanage.mesh.get_shape())
    tool.save_refs(r"M:\pycharm\Inversion\data\Temp\refs", -10, 2.4, 1.89)
